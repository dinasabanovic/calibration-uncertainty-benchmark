...
revision_night1.py
------------------
Night 1 of the major revision: full re-baseline.
...
2. After every dataset, the partial CSV is written to
   <work_dir>/night1_partial.csv so a Ctrl-C / disconnect loses at most
   one dataset's progress. When the loop finishes, the partial is
...
* <work_dir>/night1_failures.csv     any rows that errored, with reasons

Usage
-----
    # from the repository root
    python experiments/revision_night1.py
    # or with a custom work dir
    python experiments/revision_night1.py --work_dir /path/to/output
...
# =============================================================================
# Config (matches the paper §3.4 and Night 1 notebook exactly)
# =============================================================================
...
def main() -> int:
    ...
    partial_ckpt = work_dir / "night1_partial.csv"
    ...
    if fail_log:
        pd.DataFrame(fail_log).to_csv(work_dir / "night1_failures.csv", index=False)
        print(f"failures: {work_dir/'night1_failures.csv'}")
