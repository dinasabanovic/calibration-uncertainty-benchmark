...
revision_night3.py
------------------
Night 3 of the major revision: TabPFN, OOD, label noise, conformal,
post-hoc analyses.
...
Usage
-----
    python experiments/revision_night3.py                       # all sections
    python experiments/revision_night3.py --section ood         # just OOD
    python experiments/revision_night3.py --section tabpfn      # just TabPFN
...
def run_ood(work_dir: Path, manifest: pd.DataFrame, device) -> None:
    ...
    partial   = work_dir / "night3_ood_partial.csv"
    ...
    if results:
        ...
        if partial.exists():
            partial.unlink()
    if failures:
        pd.DataFrame(failures).to_csv(work_dir / "night3_ood_failures.csv",
                                       index=False)
...
def run_label_noise(work_dir: Path, manifest: pd.DataFrame, device) -> None:
    ...
    print(f"\n========== Section: Label noise ==========")
    # 6-dataset subset matching Night 3 notebook: smallest 2 per size regime
    noise_tasks = []
    ...
    partial = work_dir / "night3_noise_partial.csv"
    if partial.exists():
        prev = pd.read_csv(partial)
        results = prev.to_dict("records")
    ...
    if failures:
        pd.DataFrame(failures).to_csv(
            work_dir / "night3_noise_failures.csv", index=False,
        )
